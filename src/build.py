...
"""Build the Squatchware and Squatchware Light Omarchy themes.
    ...
    python3 src/build.py            # rebuild both themes and extras/bootloader/
    python3 src/build.py --check    # contrast check only

Omarchy installs a theme from a repo whose root is the theme, so the two variants live in two
repos: the dark theme is this repo's root, and the light theme is written to the sibling checkout
../omarchy-squatchware-light-theme (override with SQUATCHWARE_LIGHT_REPO).
...
ROOT = HERE.parent
BRAND = Path(os.environ.get("SQUATCHWARE_BRAND", ROOT.parent / "squatchware.dev" / "brand"))
LIGHT_REPO = Path(os.environ.get("SQUATCHWARE_LIGHT_REPO", ROOT.parent / "omarchy-squatchware-light-theme"))
...
def build_theme(pal, out, readme=None):
...
LIGHT_README = """# Squatchware Light for Omarchy
...
![Desktop](preview.png)
![Lock screen and boot splash](preview-unlock.png)
...
def main():
    ...
    build_theme(DARK, ROOT)  # the dark repo's README is hand-written
    build_theme(LIGHT, LIGHT_REPO, LIGHT_README)
    build_bootloader()
    print(f"built {ROOT}, {LIGHT_REPO}, extras/bootloader/")
